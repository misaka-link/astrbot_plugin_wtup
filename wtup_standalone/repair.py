...
async def parse_or_repair_analysis_with_usage(
    ...
    *,
    ...
    logger.warning("[%s] 模型输出 JSON 解析失败，启动 JSON 修复模型请求 chunk %d/%d", "wtup_standalone", chunk.index, chunk.total)
    ...
    try:
        ...
        logger.warning("[%s] JSON 修复模型请求仍未返回有效 JSON chunk %d/%d", "wtup_standalone", chunk.index, chunk.total)
        ...
        logger.warning("[%s] JSON 修复模型请求失败 chunk %d/%d: %s", "wtup_standalone", chunk.index, chunk.total, exc)
