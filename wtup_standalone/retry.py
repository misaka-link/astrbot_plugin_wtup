...
from .errors import classify_model_error, record_model_error
...
async def refine_merged_analysis_with_usage(
    ...
    try:
        ...
        logger.warning("[%s] 总结模型分析失败，使用程序合并结果: %s", "wtup_standalone", exc)
...
async def analyze_chunk_with_retry_attempt(
    ...
    *,
    ...
    try:
        ...
        if error_info.deterministic:
            record_model_error(
                settings,
                "chunk_analysis_deterministic_failed",
                exc,
                summary=summary,
                chunk=chunk,
                extra={
            ...
            logger.warning(
                "[%s] Provider %s 模型请求确定性失败(%s)，跳过拆分重试 chunk %d/%d: %s",
                "wtup_standalone",
                provider_label(provider_id),
                error_info.category,
                chunk.index,
        ...
        try:
            ...
            logger.warning("[%s] 拆分重试结果合并失败 chunk %d/%d: %s", "wtup_standalone", chunk.index, chunk.total, merge_exc)
            return ChunkAnalysis(
                chunk.index,
                chunk.total,
                enforce_change_coverage(
...
def _merge_dynamic_parent_results(
    ...
    for chunk, original_result in zip(chunks, ordered_results):
        ...
        try:
            ...
            logger.warning("[%s] 动态补充结果合并失败 chunk %d/%d: %s", "wtup_standalone", chunk.index, chunk.total, exc)
...
def _record_task_log(settings: PluginConfig, event: str, metadata: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning("[%s] 写入动态补充任务日志失败: %s", "wtup_standalone", exc)
...
async def refine_chunk_analyses_with_usage(
    ...
    *,
    ...
    try:
        ...
        logger.warning("[%s] 分片总结模型分析失败，使用兜底报告: %s", "wtup_standalone", exc)
        return (
            enforce_change_coverage(
                summary,
                chunks,
