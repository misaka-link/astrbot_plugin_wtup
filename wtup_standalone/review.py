...
from .config import AnalyzerConfig as PluginConfig, REVIEW_MODE_AUTO, REVIEW_MODE_ENERGY, REVIEW_MODE_QUALITY
from .diff_collector import DiffSummary
from .client import request_llm
from .coverage import collect_source_ids, enforce_change_coverage, expected_source_ids
...
async def review_analysis_with_usage(
    ...
    expected_ids = expected_source_ids(summary, summary.chunks)
    if not getattr(settings, "enable_review_model", False):
        normalized, _prefilter_stats = _filter_invalid_source_items(
            normalized,
            expected_ids,
            requested_filters=[],
        )
        normalized = enforce_change_coverage(summary, summary.chunks, normalized)
    ...
    issues = _programmatic_review_issues(normalized, expected_ids)
    ...
    for round_index in range(1, rounds + 1):
        ...
        try:
            ...
            logger.warning("[%s] 监督模型复核失败，保留当前报告: %s", "wtup_standalone", exc)
        ...
        if mode == REVIEW_MODE_QUALITY:
            ...
            filtered_items = _review_payload_filtered_items(payload)
            if filtered_items:
                current, filter_stats = _filter_invalid_source_items(
                    current,
                    expected_ids,
                    requested_filters=filtered_items,
                )
                revision_stats = _merge_revision_stats(revision_stats, filter_stats)
                if filter_stats["filtered"]:
                    applied_revision = True
                    round_changed = True
                if filter_stats["rejected"]:
                    round_rejected = True
                issues = _unique([*issues, *filter_stats["issues"], *filter_stats["rejected_reasons"]])

            revised = payload.get("revised_analysis") if isinstance(payload.get("revised_analysis"), dict) else None
            if revised is not None and not revised_items and not filtered_items:
    ...
    current, final_filter_stats = _filter_invalid_source_items(
        current,
        expected_ids,
        requested_filters=[],
    )
    if final_filter_stats["filtered"] or final_filter_stats["rejected"]:
        revision_stats = _merge_revision_stats(revision_stats, final_filter_stats)
        issues = _unique([*issues, *final_filter_stats["issues"], *final_filter_stats["rejected_reasons"]])
        if final_filter_stats["filtered"]:
            applied_revision = True
    current = enforce_change_coverage(summary, summary.chunks, current)
...
5. 允许过滤疑似幻觉内容：只能在 filtered_items 中按已有 item_id 指定需要移出正文的条目，不能新增、删除或改写其他内容。
6. {revision_text}
7. 不确定如何修正时，只写入 issues，不要硬改。
...
{{
  ...
  "filtered_items": [
    {{"item_id": "I006", "reason": "缺少 source_ids 或来源编号不在 expected_source_ids 中"}}
  ]
}}
...
- 如果条目缺少 source_ids，或 source_ids 不在 expected_source_ids 中，视为疑似幻觉；质量档可通过 filtered_items 过滤该条目。
...
def _programmatic_review_issues(analysis: dict[str, Any], expected_ids: set[str] | None = None) -> list[str]:
    issues: list[str] = []
    coverage = analysis.get("coverage") if isinstance(analysis.get("coverage"), dict) else {}
    missing = _as_int(coverage.get("missing"))
    if missing:
        issues.append(f"仍有 {missing} 个变更编号由程序补入需复核条目。")
    invalid_items = _invalid_source_item_issues(analysis, expected_ids or set())
    issues.extend(invalid_items[:8])
...
def _invalid_source_item_issues(analysis: dict[str, Any], expected_ids: set[str]) -> list[str]:
    if not expected_ids:
        return []
    issues: list[str] = []
    counter = 0
    ...
    def visit(items: Any) -> None:
        ...
        for item in items:
            ...
            source_ids = [str(value).strip() for value in item.get("source_ids", []) if str(value or "").strip()]
            if not source_ids:
                issues.append(f"条目 {item_id} 缺少 source_ids，疑似幻觉内容。")
            else:
                invalid_ids = [source_id for source_id in source_ids if source_id not in expected_ids]
                if invalid_ids:
                    issues.append(
                        f"条目 {item_id} 的 source_ids 不在 expected_source_ids 中："
                        + "、".join(invalid_ids[:8])
                        + "，疑似幻觉内容。"
                    )
...
def _review_payload(
    ...
    *,
    ...
    expected = sorted(expected_source_ids(summary, summary.chunks))
...
def _review_payload_filtered_items(payload: dict[str, Any]) -> list[dict[str, str]]:
    raw = payload.get("filtered_items")
    if raw is None:
        raw = payload.get("hallucinated_items")
    ...
    for item in raw:
        ...
        if not item_id:
            continue
        reason = str(item.get("reason") or "监督模型标记为疑似幻觉内容").strip()
        result.append({"item_id": item_id, "reason": reason})
    return result


def _filter_invalid_source_items(
    analysis: dict[str, Any],
    expected_ids: set[str],
    *,
    requested_filters: list[dict[str, str]],
) -> tuple[dict[str, Any], dict[str, Any]]:
    updated = normalize_analysis(analysis)
    stats = _empty_revision_stats()
    requested_by_id = {
        str(item.get("item_id") or "").strip().upper(): str(item.get("reason") or "").strip()
        for item in requested_filters
        if isinstance(item, dict) and str(item.get("item_id") or "").strip()
    }
    stats["filter_attempted"] = len(requested_by_id)
    rejected_reasons: list[str] = []
    hallucinations = list(updated.get("suspected_hallucinations") or [])
    counter = 0
    matched_requested: set[str] = set()
    auto_check = bool(expected_ids)

    def visit(items: Any, section_title: str) -> list[dict[str, Any]]:
        nonlocal counter, hallucinations
        if not isinstance(items, list):
            return []
        ...
        for item in items:
            ...
            source_ids = [str(value).strip() for value in item.get("source_ids", []) if str(value or "").strip()]
            reasons: list[str] = []
            requested_reason = requested_by_id.get(item_id)
            if requested_reason is not None:
                matched_requested.add(item_id)
                reasons.append(requested_reason or "监督模型标记为疑似幻觉内容")
            if auto_check:
                if not source_ids:
                    reasons.append("缺少 source_ids")
                else:
                    invalid_ids = [source_id for source_id in source_ids if source_id not in expected_ids]
                    if invalid_ids:
                        reasons.append("source_ids 不在 expected_source_ids 中：" + "、".join(invalid_ids[:8]))
            if reasons:
                ...
                stats["filtered"] += 1
                continue
            children = visit(item.get("children"), section_title)
            normalized_item = dict(item)
            normalized_item["children"] = children
            kept.append(normalized_item)
        return kept

    sections = updated.get("update_sections") if isinstance(updated.get("update_sections"), list) else []
    filtered_sections: list[dict[str, Any]] = []
    for section in sections:
        if not isinstance(section, dict):
            continue
        title = str(section.get("title") or "").strip()
        items = visit(section.get("items"), title)
        if items:
            filtered_sections.append({"title": title, "items": items})

    for item_id in sorted(set(requested_by_id) - matched_requested):
        rejected_reasons.append(f"过滤条目 {item_id} 不存在，已拒绝。")

    stats["rejected_reasons"] = _unique(rejected_reasons)
    stats["rejected"] = len(stats["rejected_reasons"])
    if stats["filtered"]:
        stats["issues"] = [f"已将 {stats['filtered']} 条疑似幻觉内容移入独立分区。"]
    else:
        stats["issues"] = []
    updated["update_sections"] = filtered_sections
    updated["suspected_hallucinations"] = hallucinations
    return normalize_analysis(updated), stats
...
def _merge_revision_stats(base: dict[str, Any], extra: dict[str, Any]) -> dict[str, Any]:
    ...
    merged["filtered"] = int(merged.get("filtered") or 0) + int(extra.get("filtered") or 0)
    merged["filter_attempted"] = int(merged.get("filter_attempted") or 0) + int(extra.get("filter_attempted") or 0)
    merged["rejected_reasons"] = _unique(
        [*list(merged.get("rejected_reasons") or []), *list(extra.get("rejected_reasons") or [])]
    )
    merged["issues"] = _unique([*list(merged.get("issues") or []), *list(extra.get("issues") or [])])
