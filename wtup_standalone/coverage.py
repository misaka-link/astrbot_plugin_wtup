...
def expected_source_ids(summary: DiffSummary, chunks: list[DiffChunk]) -> set[str]:
    return {entry.source_id for entry in _expected_entries(summary, chunks)}
...
def _classify_compacted_missing_items(groups: list[CompactedChangeGroup]) -> dict[str, list[dict[str, Any]]]:
    cross_file_groups = _compact_cross_file_groups(groups)
    cross_file_source_ids = {
        source_id
    ...
    for group in cross_file_groups:
        result["batch"].append(_cross_file_group_item(group))

    for group in groups:
        if all(source_id in cross_file_source_ids for source_id in group.source_ids):
            continue
...
def _compact_cross_file_groups(groups: list[CompactedChangeGroup]) -> list[CrossFileChangeGroup]:
    by_summary: dict[str, list[CompactedChangeGroup]] = {}
    for group in groups:
        by_summary.setdefault(group.summary, []).append(group)

    result: list[CrossFileChangeGroup] = []
    for summary, same_summary_groups in by_summary.items():
        object_names: list[str] = []
        source_ids: list[str] = []
        for group in same_summary_groups:
            object_name = _object_name_from_path(group.file_path)
            if object_name not in object_names:
                object_names.append(object_name)
            for source_id in group.source_ids:
                if source_id not in source_ids:
                    source_ids.append(source_id)
        if len(object_names) < 2 or len(source_ids) < 2:
            continue
        result.append(CrossFileChangeGroup(summary=summary, object_names=object_names, source_ids=source_ids))
    return result
...
def _cross_file_group_item(group: CrossFileChangeGroup) -> dict[str, Any]:
    names = _format_object_names(group.object_names)
    summary = _ensure_sentence(group.summary)
    return {
        "text": f"{names} 修改了同一内容：{summary}",
        "children": [],
        "source_ids": group.source_ids,
    }


def _format_object_names(names: list[str]) -> str:
    normalized = [name for name in names if name]
    if len(normalized) <= 6:
        return "、".join(normalized)
    return "、".join(normalized[:6]) + f" 等 {len(normalized)} 项"


def _object_name_from_path(path: str) -> str:
    text = str(path or "").strip().replace("\\", "/")
    filename = text.rsplit("/", 1)[-1] if text else "未知对象"
    if "." in filename:
        filename = filename.rsplit(".", 1)[0]
    return filename or text or "未知对象"
...
def _file_entries(file_info: dict[str, Any], file_index: int) -> list[ChangeEntry]:
    ...
    if file_info.get("csv_subdivided"):
        groups = file_info.get("csv_groups") if isinstance(file_info.get("csv_groups"), list) else []
        entries: list[ChangeEntry] = []
        for index, group in enumerate(groups, start=1):
            ...
            sample_keys = group.get("sample_keys") if isinstance(group.get("sample_keys"), list) else []
            sample = "、".join(str(key)[:60] for key in sample_keys[:2])
            description = f"localization 类别 {name}：{count} 行"
            if sample:
                description += f"（代表: {sample}）"
            entries.append(
                ChangeEntry(
                    source_id=f"C{file_index:03d}-{index:03d}",
                    file_path=path,
                    status=status,
        ...
        return [
            ChangeEntry(
                ...
                file_path=path,
                status=status,
                description="多语言 localization CSV（已按行归类），需复核新增内容。",
            )
        ]
