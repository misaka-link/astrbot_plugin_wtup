...
class GitHubCache:
    ...
    def get_file_bytes(
        ...
        """按字节缓存文件内容（二进制安全），供结构化对比拉取新旧版本使用。"""
        cache_path, metadata_path = self._binary_file_paths(repo, ref, path)
        hit, data = self._read_bytes(cache_path)
        if hit:
            self._record(
                "GitHub 缓存命中",
                "blob",
            ...
            return CacheResult(data, "cache", cache_path)

        self._record("GitHub 缓存未命中", "blob", repo=repo, ref=ref, target_path=path, path=cache_path)
        data = fetcher()
        content_saved = self._write_bytes(cache_path, data)
        metadata_saved = self._write_json(
            metadata_path,
            {
                "repo": repo,
        ...
        return CacheResult(data, "github", cache_path)
    ...
    def _binary_file_paths(self, repo: str, ref: str, path: str) -> tuple[Path, Path]:
        digest = hashlib.sha256(f"blob\0{repo}\0{ref}\0{path}".encode("utf-8")).hexdigest()
        directory = self.root / "blobs" / _safe_key(ref)
        return directory / f"{digest}.bin", directory / f"{digest}.json"

    def _read_bytes(self, path: Path) -> tuple[bool, bytes]:
        if not path.exists():
            return False, b""
        try:
