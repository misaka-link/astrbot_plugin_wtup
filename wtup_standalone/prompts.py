...
from .config import AnalyzerConfig as PluginConfig
...
def _analysis_json_schema_text(enable_ai_analysis: bool = False) -> str:
    ai_schema_part = ""
...
def build_prompt(settings: PluginConfig, summary: DiffSummary, chunk: DiffChunk) -> str:
    enable_ai = getattr(settings, "enable_ai_analysis", False)
    ai_rule_2 = (
        "2. 输出内容格式参考 War Thunder Datamine 更新日志：先按条目列出更新内容，再在下面列出 AI 分析的改动内容。"
        if enable_ai
        else "2. 输出内容格式参考 War Thunder Datamine 更新日志：按条目列出客观具体的更新条目，专注于数据与文件变动，无需输出 ai_analysis 战术推演。"
    )
    ai_rule_8 = (
        "8. update_sections 是完整更新条目，不受 AI 分析摘要数量限制；changed_content、player_impact、uncertainties 每个数组最多 50 条，每条尽量短。"
        if enable_ai
        else "8. update_sections 是完整更新条目，必须详尽列出所有提取到的改动项，无需生成 ai_analysis。"
    )
...
JSON 字段如下：
{_analysis_json_schema_text(enable_ai)}
...
19. 多个文件、载具、武器或对象修改同一字段/同一内容时，合并成一条“对象 A、对象 B 修改了同一内容：xxx”的摘要，不要逐条铺开，并保留全部 source_ids。
...
7. 将批量重复内容从 update_sections 中剔出来，放入 bulk_repeat_content；这里的条目也必须保留 source_ids。多个对象修改同一内容时合并成一条摘要，不要逐条铺开。
...
9. 将批量重复内容从 update_sections 中剔出来，放入 bulk_repeat_content；这里的条目也必须保留 source_ids。多个对象修改同一内容时合并成一条摘要，不要逐条铺开。
...
15. 多个文件、载具、武器或对象修改同一字段/同一内容时，合并成一条“对象 A、对象 B 修改了同一内容：xxx”的摘要，不要逐条铺开，并保留全部 source_ids。
