...
class GitHubClient:
    ...
    def get_commits(self, repo: str, branch: str = "master", per_page: int = 30) -> list[CommitRef]:
        return self.get_recent_commits(repo, branch, limit=per_page)

    get_compare = compare_commits
    get_compare_diff = compare_diff_text
    ...
    def get_blob(self, repo: str, ref: str, path: str) -> bytes:
        """按 ref 拉取文件原始字节（二进制安全，走 raw.githubusercontent.com）。"""
        safe_path = urllib.parse.quote(str(path or "").strip().lstrip("/"), safe="/")
        safe_ref = urllib.parse.quote(str(ref or "").strip(), safe="")
        url = f"{GITHUB_RAW_BASE}/{repo}/{safe_ref}/{safe_path}"
    ...
    def _request_bytes(self, raw_url: str) -> bytes:
        headers = {
        ...
        if self.token:
            headers["Authorization"] = f"Bearer {self.token}"
        return self._request_bytes_with_retry(raw_url, headers)

    def _request_bytes_with_retry(self, url: str, headers: dict[str, str]) -> bytes:
        last_error: GitHubRequestError | None = None
        for attempt in range(self.max_retries + 1):
            if attempt > 0:
                wait = self._retry_delay_seconds(attempt)
                ...
                self._sleep_before_retry(wait)
            try:
                return self._do_request_bytes(url, headers)
            except GitHubRequestError as exc:
                if exc.status_code is not None and exc.status_code not in RETRYABLE_STATUSES:
                    raise
                last_error = exc
    ...
    def _do_request_bytes(self, url: str, headers: dict[str, str]) -> bytes:
        request = urllib.request.Request(url, headers=headers)
        try:
            with urllib.request.urlopen(request, timeout=self.timeout) as response:
