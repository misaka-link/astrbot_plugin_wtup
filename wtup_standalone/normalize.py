...
SUSPECTED_HALLUCINATION_LIMIT = 100
...
def normalize_analysis(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "suspected_hallucinations": normalize_suspected_hallucinations(payload.get("suspected_hallucinations")),
...
def normalize_suspected_hallucinations(value: Any) -> list[dict[str, Any]]:
    if isinstance(value, list):
        items = value
    elif value:
        items = [value]
    ...
    for item in items:
        if isinstance(item, dict):
            ...
            reasons = normalize_list(item.get("reasons") or item.get("reason"), limit=10)
            normalized = {
                "text": text,
                "source_ids": source_ids,
                "reasons": reasons,
            }
            for key in ("item_id", "section"):
                value_text = clean_pagination_text(item.get(key))
                if value_text:
                    normalized[key] = value_text
        else:
            text = clean_pagination_text(item)
            normalized = {"text": text, "source_ids": [], "reasons": []}
        if not normalized["text"] or is_compound_raw_diff_summary(normalized["text"]):
            continue
        result.append(normalized)
        if len(result) >= SUSPECTED_HALLUCINATION_LIMIT:
            break
    return dedupe_suspected_hallucinations(result)

def dedupe_suspected_hallucinations(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    result: list[dict[str, Any]] = []
    ...
    for item in items:
        ...
        if text in index_by_text:
            ...
            existing["source_ids"] = unique_texts([*existing.get("source_ids", []), *item.get("source_ids", [])])
            existing["reasons"] = unique_texts([*existing.get("reasons", []), *item.get("reasons", [])])
            continue
        normalized = {
            "text": text,
            "source_ids": unique_texts(item.get("source_ids", [])),
            "reasons": unique_texts(item.get("reasons", [])),
        }
        for key in ("item_id", "section"):
