...
class AnalysisResultCache:
    ...
    def read(self, *, settings: PluginConfig, repo: str, summary: DiffSummary) -> AnalysisCacheEntry | None:
        ...
        try:
            ...
            _logger.warning("[%s] 分析结果缓存读取失败: %s (%s)", "wtup_standalone", path, exc)
    ...
    def write(
        ...
        *,
        ...
        try:
            ...
            _logger.warning("[%s] 分析结果缓存写入失败: %s (%s)", "wtup_standalone", path, exc)
    ...
    def _record(self, event: str, *, repo: str, summary: DiffSummary, key: str, path: Path) -> None:
        ...
        try:
            ...
            _logger.warning("[%s] 写入分析结果缓存任务日志失败: %s", "wtup_standalone", exc)
