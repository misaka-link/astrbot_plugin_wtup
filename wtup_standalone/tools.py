...
async def _execute_single_tool_call(
    ...
    *,
    ...
    elif tool == "get_vehicle_specs":
        result = _get_vehicle_specs_tool(settings, summary, call, round_index=round_index)
    elif tool == "read_file_section":
        result = _read_file_section_tool(settings, summary, call, round_index=round_index)
    else:
...
def _record_tool_call(settings: PluginConfig, result: dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning("[%s] 写入工具调用日志失败: %s", "wtup_standalone", exc)
...
def _get_vehicle_specs_tool(
    ...
    *,
    ...
    vehicle_id = str(call.get("vehicle_id") or call.get("path") or "").strip().lower()

    # 从 summary files 中组装 diff 片段
    relevant_diff = []
    for f in summary.files:
        p = str(f.get("patch") or "")
        fn = str(f.get("filename") or "")
        if "wpcost" in fn or "shop" in fn or (vehicle_id and vehicle_id in fn):
            relevant_diff.append(f"diff --git a/{fn} b/{fn}\n{p}")

    facts = extractor.extract_facts("\n".join(relevant_diff), summary)
    res_data = []
    for v in facts.new_vehicles:
        if not vehicle_id or vehicle_id in v.unit_id.lower() or vehicle_id in v.display_name.lower():
            res_data.append({
                "unit_id": v.unit_id,
                ...
                "tier": v.tier,
                "br": {
                    "ab": v.br.ab, "air_rb": v.br.air_rb, "ground_rb": v.br.ground_rb,
                    "naval_rb": v.br.naval_rb, "sb": v.br.air_sb
                },
                "radar": v.radar,
                "rwr": v.rwr,
                "countermeasures": v.countermeasures,
                "custom_loadouts": v.custom_loadouts,
            })

    content = json.dumps(res_data, ensure_ascii=False, indent=2) if res_data else f"未在当前变更中找到载具 {vehicle_id} 的规格数据"
    return _tool_result(call, status="success" if res_data else "not_found", content=_truncate(settings, content), round_index=round_index)


def _read_file_section_tool(
    settings: PluginConfig,
    summary: DiffSummary,
    call: dict[str, Any],
    *,
    ...
    path = _clean_path(call.get("path"))
    keyword = str(call.get("keyword") or call.get("query") or "").strip().lower()
    if not path:
        return _tool_result(call, status="rejected", content="read_file_section 需要提供 path。", round_index=round_index)

    file_info = _file_by_path(summary, path)
    if file_info is None:
        return _tool_result(call, status="not_found", content=f"本次 diff 不包含 {path}", round_index=round_index)
    ...
    if keyword:
        for idx, line in enumerate(lines):
            if keyword in line.lower():
                ...
                matching_lines.append(f"--- 匹配附近 (行 {start+1}~{end}) ---\n" + "\n".join(lines[start:end]))
        content = "\n\n".join(matching_lines[:3]) if matching_lines else f"在 {path} 中未匹配到关键词 {keyword}"
    else:
        content = "\n".join(lines[:100])

    return _tool_result(call, status="success", content=_truncate(settings, content), round_index=round_index)
