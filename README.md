...
- `analysis_prompt`：分析提示词，默认要求按 War Thunder Datamine 更新日志风格覆盖全部改动，补充中文说明和实际影响，并把批量/重复/需验证内容分流到报告下方。
- `summary_prompt`：总结提示词，用于总结模型整理全部分片分析结果，默认同样要求不遗漏分片中的任何改动，并保留批量/重复/需验证内容分流。
...
- `review_mode`：监督模型预设，默认 `quality` 质量档。`off` 为关闭，`energy` 为节能档，`quality` 为质量档，`auto` 为自动档。节能档只查漏编号、格式和模糊表述；质量档还会更细地检查分类、参数说明和影响描述，并可在保留 `source_ids` 的前提下采用局部文字修正。
...
- `review_prompt`：监督模型提示词，默认要求监督模型只做质检，不重新创作整篇报告；质量档如需修正文案，会通过 `item_revisions` 返回局部文字修正。
...
强制分析最新 commit，用于测试图片渲染和模型分析。默认分析最新 1 个 commit；命令末尾可追加数量参数，例如 `/wtup_check 强制 2` 会分析最新 2 个 commit，允许范围为 1 到 5。命令会直接发送到当前触发命令的群，不使用后台配置的 `target_groups` 和 `analysis_file_groups` 列表；如果无法识别当前群号，会提示失败。开启 `enable_pre_summary_report` 且总结模型启用时，当前群会依次收到分析前和分析后的两份报告，并上传两份 `.log` 分析日志；开启 `enable_push_append_text` 时，每份报告图片后都会追加对应文字内容。
...
插件会为每个分片生成“变更覆盖点名册”，按文件和连续 diff 变更块分配 `C001-001` 这类编号，并在模型提示词中要求每条 `update_sections.items`、子条目或 `bulk_repeat_content` 条目写入对应 `source_ids`。模型返回、JSON 修复、工具补充、动态补充和总结模型整理后，程序都会保留这些编号。

最终报告生成前还会执行一次覆盖检查：如果某个原始变更编号没有出现在任何报告条目的 `source_ids` 中，插件会把漏掉的原始变更按文件和 diff 摘要补回 `bulk_repeat_content`，并在 `coverage` 统计和任务日志“变更覆盖检查”中记录应覆盖、已覆盖和未覆盖数量。兜底条目会把同一文件内相同旧值/新值的机械变更归入“重复内容”，把跨文件同类摘要归入“批量修改”，其他无法解释的批量或机械内容归入“需验证内容”；这些内容显示在报告更新内容下方、AI 分析上方，不再把 `@@` hunk 行号、`删除/旧值`、`新增/新值` 等原始点名册文本逐条铺到主要更新正文。连续新增/删除的对象、数组或字典复合块会被视为低信息密度的结构噪声，不进入覆盖点名册和未覆盖兜底报告。如果模型或总结模型自己照抄了这类原始 diff 摘要，标准化阶段也会先改写、去重或过滤后再进入最终报告。这样模型可以解释得不够好，但不能静默漏掉可读的原始改动。

报告标题不再完全依赖模型输出。插件会从 GitHub compare 的 base commit 和 head commit 提交信息中提取版本号；如果模型把 `report_title` 写成 `-> 2.56.0.43`、`2.56.0.43` 或其他缺边标题，最终输出、图片标题、日志文件名和追加文字会优先补成完整的 `旧版本->新版本`。
...
`enable_pre_summary_report` 默认关闭。开启后且 `enable_summary_model` 也开启时，插件会保留总结模型处理前的程序合并报告，并在总结模型处理后再生成最终报告。两份报告都会渲染图片、保存日志；`/wtup_check 强制` 会依次把两份报告和两份日志文件发送到当前群；如果配置了群推送，会依次发送两张报告图；如果配置了 `analysis_file_groups`，也会上传两份 `.log` 文件。只要本次分析成功并生成报告文件，就会标记本次更新完成；后续图片、追加文字或日志文件推送失败只记录结果，不再让下次循环重复分析同一 commit。两份报告的图片和文本里会标注“总分析模型分析前 / 总分析模型分析后”，日志文件名会追加 `_总分析前` 和 `_总分析后`，避免同一版本范围互相覆盖。

`enable_review_model` 默认关闭。开启后，最终报告生成前会再过一层监督复核。监督模型分三种预设：`energy` 只做“点名查作业”，主要检查有没有漏编号、JSON 结构、模糊表述和必要影响说明；`quality` 是默认档，会更细地检查条目解释、分类和参数含义；`auto` 平时走节能档，发现漏项较多或动态补充失败时自动升级到质量档。

质量档默认采用局部修正协议：程序会给待审条目分配 `I001`、`I002` 这类临时 `item_id`，并优先把模糊表述、需复核提示、短文本覆盖多个 `source_ids`、武器/弹药/导弹/雷达/经济等高风险条目交给监督模型。监督模型只能通过 `item_revisions` 返回某个 `item_id` 的新文本，程序只替换原条目的 `text`，保留原来的 `source_ids`、子条目、分类和顺序。如果模型返回整篇 `revised_analysis`，插件仍会走旧兼容校验；只要丢失 `source_ids` 就拒绝采用。
