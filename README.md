...
最终报告生成前还会执行一次覆盖检查：如果某个原始变更编号没有出现在任何报告条目的 `source_ids` 中，插件会自动追加“需复核的未覆盖变更”分类，把漏掉的原始变更按文件和 diff 摘要补回报告，并在 `coverage` 统计和任务日志“变更覆盖检查”中记录应覆盖、已覆盖和未覆盖数量。兜底条目会把同一文件内相同旧值/新值的机械变更合并成一条摘要，不再把 `@@` hunk 行号、`删除/旧值`、`新增/新值` 等原始点名册文本逐条铺到报告正文；如果模型或总结模型自己照抄了这类原始 diff 摘要，标准化阶段也会先改写和去重再进入最终报告。这样模型可以解释得不够好，但不能静默漏掉原始改动。
